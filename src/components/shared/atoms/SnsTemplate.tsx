...
export interface SnsTemplateProps {
	message: string;
}

/**
 * Sns Template
 * @props SnsTemplateProps
 */
const SnsTemplate = ({ message }: SnsTemplateProps) => {
	return (
		<div className="relative rounded-[32px] bg-white shadow-[0px_4px_4px_rgba(0,0,0,0.25)] h-[75vh] aspect-[1/2] overflow-hidden flex flex-col">
			{/* Header */}
			<div className="h-1/5 relative -mt-2 flex flex-col items-start gap-[2px] flex-shrink-0 border-b border-gray-300 bg-blue-500 backdrop-blur-[10px] px-4 py-2 w-full rounded-t-[32px]">
				...
				<div className="relative flex items-center w-full h-full m-0 p-0">
					...
					<div className="relative z-10">
						<ChevronLeft color="white" strokeWidth={2} size={20} />
					</div>
					<div className="text-white text-center font-inter text-base font-medium leading-normal tracking-[0.18px]">
						TEL4VN
					</div>
				</div>
			</div>
			...
			{/* Content (Flex-Grow) */}
			<div className="flex-1 overflow-auto px-4 bg-gray-200">
				<div className="w-full mt-5 text-[#0C0C0C] text-center font-source-serif text-[12px] font-normal leading-normal tracking-[0.18px] ">
					<span className="bg-gray-400 rounded-[10px] p-1"> Today 9:41</span>
				</div>
				<div className="m-2 space-y-3">
					<div className="relative max-w-xs p-3 bg-gray-300 rounded-lg w-5/6  bg-white">
